...
import { UserCircle, Calendar, Zap, Sparkles, Loader2, X, RefreshCw } from 'lucide-react';
...
export function ConnectionGridCard({ account, onDelete, isDeleting, onEditConnection, onSyncCatalog }: ConnectionGridCardProps) {
        ...
        <CardContent className="p-0 h-full flex flex-col">
          <div className="flex flex-col gap-6 flex-1">
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              {/* Action Button */}
              <div className="flex flex-row justify-end pt-6 border-t border-slate-100 dark:border-slate-800 gap-2">
                {onEditConnection && (account.serviceName?.toLowerCase().includes('postgres') || account.serviceName?.toLowerCase().includes('mysql')) && (
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => onEditConnection(account)}
                    className="h-12 w-12 rounded-2xl border-blue-600/20 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-all font-bold shadow-lg shadow-blue-500/10"
                    title="Edit Connection"
                  >
                    <UserCircle className="h-5 w-5" />
                  </Button>
                )}
                {onSyncCatalog && (account.serviceName?.toLowerCase().includes('postgres') || account.serviceName?.toLowerCase().includes('mysql')) && (
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => onSyncCatalog(account)}
                    className="h-12 w-12 rounded-2xl border-emerald-600/20 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-all font-bold shadow-lg shadow-emerald-500/10"
