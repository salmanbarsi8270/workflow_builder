    ...
    return (
        ...
        >
            <div className="w-80 flex flex-col h-full shrink-0">
                    ...
                    {Object.entries(groupedSessions).map(([groupName, groupSessions]) => {
                        ...
                        return (
                            <div key={groupName} className="mb-6">
                                <div className={cn(
                                    "px-3 py-2 text-[11px] font-bold uppercase tracking-widest flex items-center gap-2 mb-1",
                ...
                {/* Footer */}
                <div className="p-3 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-[#0a0a0a]/50">
                    <Button
                        variant="ghost"
                        size="sm"
    ...
    return (
        <div className="group/item relative px-1">
            <button
                ...
                className={cn(
                    "h-10 px-3 py-2 rounded-xl transition-all border flex items-center gap-3 w-full group/btn text-left",
                    isSelected 
            ...
            >
                {isEditing ? (
                    ...
                    <div className="flex items-center gap-3 w-full min-w-0 pr-6">
                        ...
                        <span className="truncate text-sm font-semibold flex-1">{session.title}</span>
                    </div>
                )}
            </button>

            {!isEditing && (
                <div className="absolute right-3 top-1/2 -translate-y-1/2 items-center gap-0.5 hidden group-hover/item:flex transition-opacity bg-white/90 dark:bg-[#0a0a0a]/90 backdrop-blur-sm p-1 rounded-lg border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-none z-10">
                    <button 
