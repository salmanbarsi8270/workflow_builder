...
import { Loader2, Settings, Play, Sparkles, Code2, Lightbulb } from "lucide-react"
...
export function CreateActionDialog({ open, onOpenChange, connectorId, initialData }: CreateActionDialogProps) {
    const { user } = useUser();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success(isUpdate ? "Action updated successfully!" : "Action created successfully!");
            onOpenChange(false);
                    ...
                    <DialogHeader className="border-b border-slate-200 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/10 p-6 backdrop-blur-sm">
                        <div className="space-y-2">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-lg bg-linear-to-br from-violet-500 to-violet-600 shadow-lg">
                                    <Play className="h-5 w-5 text-white" />
                                </div>
                                <DialogTitle className="text-2xl font-bold bg-linear-to-r from-violet-600 to-blue-600 dark:from-violet-400 dark:to-blue-400 bg-clip-text text-transparent">
                                    {isUpdate ? 'Update Action' : 'Create Action'}
                                </DialogTitle>
                                {isUpdate && (
                            ...
                            <DialogDescription className="text-slate-500 dark:text-slate-400 ml-11">
                                {isUpdate ? `Update existing action: ${initialData?.name}` : 'Define a new action for a connector.'}
                            </DialogDescription>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="max-h-[65vh] overflow-y-auto p-6 custom-scrollbar space-y-5">
                        ...
                        <div className="space-y-2.5 group">
                            <Label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-300 flex items-center gap-2">
                                <Code2 className="h-3.5 w-3.5 text-violet-500 dark:text-violet-400" />
                                Select Connector
                            </Label>
                            <Select 
                                disabled
                            ...
                            >
                                <SelectTrigger className={cn(
                                    "h-12 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-foreground transition-all duration-300 hover:border-violet-500/50 focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800",
                                    focusedField === 'connector' && "border-violet-500 bg-white dark:bg-slate-800"
                                )}
                                    onFocus={() => setFocusedField('connector')}
                                    onBlur={() => setFocusedField(null)}
                                >
                                    <div className="flex items-center gap-2">
                                        {connecting && <Loader2 className="h-4 w-4 animate-spin text-violet-500" />}
                                ...
                                <SelectContent className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-foreground">
                                    {connecting ? (
                                        <div className="flex items-center justify-center p-4">
                                            <Loader2 className="h-4 w-4 animate-spin mr-2 text-violet-500 dark:text-violet-400" />
                                            <span className="text-sm text-slate-500 dark:text-slate-400">Loading connectors...</span>
                                        </div>
                                    ) : (
                                        connectors.map(c => (
                                            <SelectItem key={c.id} value={c.id} className="hover:bg-violet-500/10 dark:hover:bg-violet-500/20">{c.name} ({c.id})</SelectItem>
                                        ))
                                    )}
                                </SelectContent>
                            </Select>
                        ...
                        <div className="space-y-2.5">
                            ...
                            <p className="text-xs text-slate-400 dark:text-slate-500 flex items-center gap-1">
                                <Lightbulb className="h-3 w-3" />
                                Ensure valid JSON format for successful action creation
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-900/50 backdrop-blur-sm p-6 flex gap-3">
                        ...
                        <Button
                            className={cn(
                                ...
                                isFormValid && !loading
                                    ? "bg-linear-to-r from-violet-500 to-violet-600 hover:from-violet-400 hover:to-violet-500 text-white shadow-lg hover:shadow-xl"
