...
export const ActivityFeed = ({ title, activities = [], className, span, rowSpan }: any) => {
  const spanClass = span ? (typeof span === 'string' ? span : `col-span-${span}`) : 'col-span-12';
  const rowSpanClass = rowSpan ? `row-span-${rowSpan}` : '';

  return (
    <div className={cn("w-full bg-background rounded-lg border shadow-sm", spanClass, rowSpanClass, className)}>
      {title && (
